  ...
  //  private static BMP theBMP = null;
    ...
    private void readToFile() throws IOException {
        ...
        for(int i=0;i<imageHeight;i++)
        {
            for(int j=0;j<imageWidth;j++)
            {
                builder.append("At index("+i+","+j+")="+quantizedCoefsYChannel[i][j]+":"+quantizedCoefsCoChannel[i][j]+":"+quantizedCoefsCgChannel[i][j]+"\n");

            }
        }
        BufferedWriter writer= new BufferedWriter(new FileWriter("QuantizedResult"+numOfInvokations+".txt"));
    ...
    private void convertYCgCoToRGB()
    {
        ...
        for(int i=0;i<imageHeight;i++)
        {
            for(int j=0;j<imageWidth;j++)
            {
                int[] rgb= convertBackToRGB(CgBufferConverted[i][j],YBufferConverted[i][j], CoBufferConverted[i][j]);
...
//    }
    ...
    {
    quantizationTable= new int[][]{ {1,1,2,4,8,16,32,64},
                                    {1,1,2,4,8,16,32,64},
                                    {2,2,2,4,8,16,32,64},
                                    {4,4,4,4,8,16,32,64},
                                    {8,8,8,8,8,16,32,64},
