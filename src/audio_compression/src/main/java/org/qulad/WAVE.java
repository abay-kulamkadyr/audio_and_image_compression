...
public class WAVE {
   ...
   //private static WAVE theWave=null;
    ...
    public WAVE(String path) {
        ...
        decoded = new AudioFormat(encoding, audioInputStream.getFormat().getSampleRate(),
                audioInputStream.getFormat().getSampleSizeInBits(),
                numChannels, bytesPerFrame, frameRate, false);
        ...
        totalSizeInBits= (long) totalNumberOfSamples *audioInputStream.getFormat().getSampleSizeInBits();
        numOfInvokations++;
    }
...
//    }
    public void read() throws IOException, DataFormatException {
        ...
        try (AudioInputStream audioInputStreamDecoded = AudioSystem.getAudioInputStream(decoded, audioInputStream)) {
            ...
            if(numChannels==1) {
                ...
                {
                    System.out.println("The number of bytes per sample is greater than 2, terminating the program...");
                    System.exit(1);
                }
            }
            else {
            ...
            while (audioInputStreamDecoded.read(rawData) != -1) {
                ...
                for (int i = 0; i < totalFrameLength; i++) {
                    ...
                    if (numChannels > 1) {
                        ...
                        rightChannelBytesSlice = Arrays.copyOfRange(arrayFrameSlice, bytesPerFrame / 2, arrayFrameSlice.length);
                        if(bytesPerSample==1) {
                    ...
                    {
                        System.out.println("The number of bytes per sample is greater than 2 bytes, terminating the program...");
                        System.exit(1);
                    }
                    start += bytesPerFrame;
                    end += bytesPerFrame;
    ...
    private void compressionMono() throws IOException, DataFormatException {
        ...
        if(bytesPerSample==1)
        {
            ...
            System.out.println();
            System.out.println("The first 20 elements of the uncompressed data (after transforming the uncompressed data to original input): ");
            for (int i=0;i<20;i++)
            {
    ...
    private void compressionStereo() throws IOException, DataFormatException {
        ...
        else if(bytesPerSample==2)
        {
            ...
            short[] originalLeftChannel= convertToLeftChannel(midConvertedFromBytesToShorts, sideConvertedFromBytesToShorts);
            short[] originalRightChannel=convertToRightChannel(midConvertedFromBytesToShorts, sideConvertedFromBytesToShorts);
